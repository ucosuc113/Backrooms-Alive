...
/**
 * Clasifica cualquier {@link BlockState} en su {@link ArchitecturalFunction}
 * correspondiente. Sin estado: {@link #classify} es una funcion pura.
 * Reubicado de {@code generation} a {@code classification} para que
 * {@code analysis}, {@code placement} y {@code degradation} puedan usarlo
...
public final class MemoryFunctionClassifier {
    ...
    public static ArchitecturalFunction classify(BlockState state) {
        ...
        if (block == Blocks.CHEST || block == Blocks.TRAPPED_CHEST
                || path.contains("barrel") || path.contains("shulker")) {
        ...
        if (state.getLightEmission() > 0
                || path.contains("lamp") || path.contains("lantern") || path.contains("torch")) {
            return ArchitecturalFunction.LIGHT;
        }
        if (path.contains("slab") || path.contains("table")
                || path.contains("chair") || path.contains("fence")) {
